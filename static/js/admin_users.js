/* ── Admin: Manage ResultAnalysis Users ──────────────────────────────────────
   Admin role can only create / remove ResultAnalysis accounts.
   No role dropdown — the backend always assigns ResultAnalysis.
*/
const API = {
...
async function loadUsers() {
  ...
  try {
    ...
    if (!data.success) {
      tbody.innerHTML = `<tr><td colspan="4" class="table-empty" style="color:var(--danger)">${data.error}</td></tr>`;
      return;
    }
    allUsers = data.users || [];
    renderUsers(allUsers);
  } catch (err) {
    tbody.innerHTML = `<tr><td colspan="4" class="table-empty" style="color:var(--danger)">Network error: ${err.message}</td></tr>`;
  }
}
...
function renderUsers(list) {
  ...
  tbody.innerHTML = list.map((u, i) => `
    <tr class="user-row" data-search="${u.UserName.toLowerCase()}">
      ...
      <td>${u.IsActive
        ...
        : '<span style="color:var(--muted);font-size:12px">Inactive</span>'}</td>
      <td>
        <button class="btn btn--danger btn--sm"
          onclick="removeUser('${u.UserId}', '${u.UserName}')">
...
async function saveUser() {
  ...
  try {
    const res  = await fetch(API.addUser, {
      ...
      // No role sent — backend always creates ResultAnalysis
...
async function removeUser(userId, username) {
  ...
  try {
    const res  = await fetch(`${API.deleteUser}?userId=${encodeURIComponent(userId)}`, { method: 'DELETE' });
    const data = await res.json();
